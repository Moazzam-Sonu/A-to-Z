...
export default function (cartId) {
    ...
    $(document.body).on('click', '[data-cart-currency-switch-url]', event => {
        const currencySessionSwitcher = event.currentTarget.href;
        const url = $(event.target).data('cart-currency-switch-url');
        const currencyCode = $(event.target).data('currency-code');
