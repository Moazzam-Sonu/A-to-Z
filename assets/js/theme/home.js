...
export default class Home extends PageManager {
    ...
    countDownBanner() {
        const $thisCountDown = $('.countdown-banner__countdown');
    ...
    scrollToNewsletter() {
        $('.btn-gt-newsletter').on('click', (event) => {
            event.preventDefault();
            $('html,body').animate({scrollTop: $('.footer-subscription').offset().top}, 700);
        })
    }
    ...
    imageComparison() {
        const handler = (entries, observer) => {
            if (entries[0].isIntersecting) {
                const imagesContainer = entries[0].target.querySelector('[data-images-container]');
        ...
        const imageComparisonBlock = document.querySelector('[data-image-comparision]');
